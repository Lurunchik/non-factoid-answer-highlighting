...
from bertviz.bertviz.neuron_view import get_attention
    ...
    while i < len(tokens):
        ...
        if weight is not None:
            if color is not None:
                orig_token = f'<span style="background-color:rgba({color},' + str(
                    weight / max_alpha) + ');">' + orig_token + '</span>'
            else:
...
def _iter_answer_tokens(att_res, ignore_tokens: Set[str] = None, merge_word_pieces: bool = False):
...
def calculate_mean_attn(att_res, ignore_tokens: Set[str] = None):
    ...
    mean_wghts = np.zeros((len(att_res['ab']['attn']), len(att_res['ab']['attn'][0]), len(filtered_tokens)))
            ...
            for head, word_weights in enumerate(data):
                ...
                for i, w in enumerate(_iter_answer_tokens(att_res, ignore_tokens)):
                    ...
                    if w.startswith('##'):
                        ...
                        mean_wghts[layer][head][token_index] = np.mean([mean_wghts[layer][head][token_index], cur_attn])
                        token_index += 1
...
class BertHighlighter:
    ...
    def from_pretrained(cls, model_path=DEFAULT_MODEL_PATH):
        return cls(tokenizer=BertTokenizer.from_pretrained(cls._bert_architecture, do_lower_case=True),
    ...
    def highlight(self, question: str, answer: str, color_fill_html: Optional[str] = '191,63,63'):
        att_res = get_attention(self._model,
                                self._model_type,
                                self._tokenizer,
                                html_replace(question), html_replace(answer))

        answer_tokens, weights = calculate_mean_attn(att_res, self._ignore_tokens)

        top_tokens = self._get_sorted_tokens(answer_tokens, weights)[:predict_highlighting_len(len(answer_tokens))]
        ...
        all_answer_tokens = list(
            _iter_answer_tokens(att_res, ignore_tokens=self.special_tokens, merge_word_pieces=True))
        return _get_text_heatmap_html(answer, all_answer_tokens, [token_weights.get(t) for t in all_answer_tokens],
                                      color_fill_html)

    def _get_sorted_tokens(self, tokens, attn, exclude_stopwords: bool = True, layer: int = None, head: int = None):
        if layer is None:
            layer = -1
